...
foreach ($candidate in $candidates) {
  try {
    if ($candidate -eq "python" -or $candidate -eq "py" -or (Test-Path -LiteralPath $candidate)) {
      & $candidate "backend/app/modules/synthetic_population/run_phase3_constraints.py"
      exit $LASTEXITCODE
    }
  } catch {}
}
throw "No usable Python interpreter found. Add Python to PATH or update scripts/run_phase3_constraints_python.ps1."
